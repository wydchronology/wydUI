...
public struct ParallaxPageView<Content: View>: ParallaxPager {
...
private struct SwipePagerRepresentable: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> SwipePagesViewController {
        let controllers = (0 ..< viewCount).map { index -> UIHostingController<AnyView> in
            let hc = UIHostingController(rootView: content(index))
            hc.view.backgroundColor = .clear
            return hc
        }
        let vc = SwipePagesViewController(controllers: controllers, configuration: configuration)
    ...
    func updateUIViewController(_ vc: SwipePagesViewController, context: Context) {
...
final class SwipePagesViewController: UIViewController {
    // MARK: Configuration
    ...
    init(controllers: [UIViewController], configuration: ParallaxPageViewConfiguration) {
