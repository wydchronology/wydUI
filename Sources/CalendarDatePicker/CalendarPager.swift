...
#Preview {
    @Previewable @State var period = Fixed<Month>(region: .autoupdatingCurrent, date: Date())

    VStack(spacing: 20) {
        Text("Infinite Calendar Carousel")
            .font(.headline)

        Text("Current Month: \(period.format(year: .naturalDigits, month: .naturalName))")
            .font(.subheadline)
            .foregroundColor(.secondary)

        CalendarPager($period) { month in
            CalendarMonthGrid(month: month.firstInstant.date) { day, _ in
                if let day = day {
                    Text("\(day)")
